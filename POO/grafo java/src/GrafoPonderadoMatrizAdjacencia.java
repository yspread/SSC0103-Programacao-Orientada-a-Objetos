...
public class GrafoPonderadoMatrizAdjacencia extends Grafo{
    ...
    public void adicionarAresta(String origem, String destino, int peso){
        ...
        matrizadjacencia.get(indexorigem).set(indexdestino, peso);
        matrizadjacencia.get(indexdestino).set(indexorigem, peso);
    }
    ...
    public boolean existeAresta(String origem, String destino){
        ...
        return (matrizadjacencia.get(indexorigem).get(indexdestino) > 0);
    }
    ...
    public int tamanho(){
        ...
        for (int linha = 0; linha < nvertices; linha++) {
            for (int coluna = linha; coluna < nvertices; coluna++) {
                if (matrizadjacencia.get(linha).get(coluna) > 0){
                    tamanho++;
                }
            }
        }
            ...
            for (int coluna = linha + 1; coluna < vertices.size(); coluna++) {
                if (matrizadjacencia.get(linha).get(coluna) > 0) {
                    ...
                    if(v1.compareTo(v2) < 0){
                        arestas.add(
                                "\"" + v1 + "\" -- \"" + v2 + "\" [label=\"" + matrizadjacencia.get(linha).get(coluna) + "\"];"
                        );
                    }
                    else {
                        arestas.add(
                                "\"" + v2 + "\" -- \"" + v1 + "\" [label=\"" + matrizadjacencia.get(linha).get(coluna) + "\"];"
                        );
                    }
                }
            }
